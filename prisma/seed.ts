...
async function main() {
  ...
  const shop = await prisma.shop.create({
    data: {
      ...
      description:
        "Your one-stop shop for all essentials, snacks, and drinks on campus.",
  ...
  const snacksCategory = await prisma.category.create({
    data: { name: "Snacks & Chips", shop_id: shop.id },
  });
  const drinksCategory = await prisma.category.create({
    data: { name: "Beverages", shop_id: shop.id },
  });
  const stationeryCategory = await prisma.category.create({
    data: { name: "Stationery", shop_id: shop.id },
  });
  ...
  const totalPrice = productsForOrder.reduce(
