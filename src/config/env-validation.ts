...
const envSchema = z.object({
  ...
  MINIO_ROOT_USER: z.string().min(3, "MINIO_ROOT_USER must be at least 3 characters"),
  ...
  AWS_SECRET_ACCESS_KEY: z.string().min(8, "AWS_SECRET_ACCESS_KEY must be at least 8 characters"),
  AWS_REGION: z.string().min(1, "AWS_REGION is required"),
  MINIO_ENDPOINT: z.string().url("MINIO_ENDPOINT must be a valid URL"),
  NEXT_PUBLIC_MINIO_ENDPOINT: z.string().url("NEXT_PUBLIC_MINIO_ENDPOINT must be a valid URL"),
...
export function checkSecurityWarnings(): void {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (process.env.NEXT_PUBLIC_MINIO_ENDPOINT?.startsWith("http://")) {
      warnings.push("NEXT_PUBLIC_MINIO_ENDPOINT should use HTTPS in production");
    }
  }
