...
export async function POST(
  ...
  try {
    ...
    await batchService.completeBatch(batch_id);

    return NextResponse.json(
      createSuccessResponse("Batch completed. All orders marked as COMPLETED.")
    );
  } catch (error) {
    console.error("POST complete-batch error:", error);
    return NextResponse.json(
      {
        error:
